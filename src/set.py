...
async def k8s_set_resources(
    ...
    Args:
        resource_type: The type of resource to modify (e.g., deployment, pod, statefulset).
        resource_name: The name of the resource to modify.
        namespace: The namespace of the resource (default: default).
        containers: List of container names to modify. If None, modifies all containers.
        limits: Resource limits as a dictionary (e.g., {"cpu": "100m", "memory": "128Mi"}).
        requests: Resource requests as a dictionary (e.g., {"cpu": "100m", "memory": "128Mi"}).
        session_id: Kubernetes session ID for remote cluster (optional)
        
    Returns:
        The JSON representation of the modified resource.
    """
    try:
        ...
        logger.error(f"Error in k8s_set_resources: {exc}")
        return {"error": str(exc)}


@mcp.tool()
async def k8s_set_image(resource_type: str, resource_name: str, container: str, image: str, namespace: str = "default", session_id: str = None):
    """
    ...
    Args:
        resource_type: The type of resource to modify (e.g., deployment, pod, statefulset).
        resource_name: The name of the resource to modify.
        container: The name of the container to modify.
    ...
    try:
        ...
        logger.error(f"Error in k8s_set_image: {exc}")
        return {"error": str(exc)}


@mcp.tool()
async def k8s_set_env(
    resource_type: str, resource_name: str, container: str, env_dict: dict, namespace: str = "default", session_id: str = None
):
    """
    ...
    try:
        ...
        if not container_found:
            return {"error": f"container '{container}' not found in resource '{resource_type}/{resource_name}'"}
        ...
        logger.error(f"Error in k8s_set_env: {exc}")
        return {"error": str(exc)}
