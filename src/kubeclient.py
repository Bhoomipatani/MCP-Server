...
@mcp.tool()
async def k8s_cordon(node_name: str, session_id: str = None):
    """
    ...
    Returns:
        The result of the cordon operation.
    """
    try:
        ...
        try:
            ...
            return {"status": "success", "message": f"Node {node_name} cordoned successfully"}

        except Exception as e:
            return {"error": str(e)}
    except Exception as exc:
        logger.error(f"Error in k8s_cordon: {exc}")
        return {"error": str(exc)}


@mcp.tool()
async def k8s_uncordon(node_name: str, session_id: str = None):
    """
    ...
    Returns:
        The result of the uncordon operation.
    """
    try:
        # Use session-specific client if provided
        if session_id:
        ...
        logger.error(f"Error in k8s_uncordon: {exc}")
        return {"error": str(exc)}
...
async def k8s_annotate(
    ...
    Args:
        resource_type: The type of resource to annotate (e.g., pods, deployments).
        name: The name of the resource to annotate. If not specified, annotates resources matching the selector.
        annotations: The annotations to add or update as a dictionary or comma-separated string (e.g., "key1=value1,key2=value2").
        namespace: The namespace of the resource (default: default).
        selector: Label selector to filter resources (e.g., "app=nginx").
        all_namespaces: Whether to annotate resources in all namespaces.
        overwrite: If true, overwrite existing annotations.
        resource_version: Only update if the resource version matches.
        dry_run: If true, only print the object that would be sent, without sending it.
        session_id: Kubernetes session ID for remote cluster (optional)
        
    Returns:
        The result of the annotate operation.
    """
    try:
        # Use session-specific client if provided
        if session_id:
        ...
        logger.error(f"Error in k8s_annotate: {exc}")
        return {"error": str(exc)}
...
@mcp.tool()
async def k8s_patch(resource_type: str, name: str, patch_data, namespace: str = "default", session_id: str = None):
    """
    Update fields of a resource using a patch.

    Args:
        resource_type: The type of resource to patch (e.g., pods, deployments).
        name: The name of the resource to patch.
        patch_data: The patch data as a JSON string or dictionary.
        namespace: The namespace of the resource (default: default).
        session_id: Kubernetes session ID for remote cluster (optional)
    ...
    try:
        ...
        logger.error(f"Error in k8s_patch: {exc}")
        return {"error": str(exc)}
...
@mcp.tool()
async def k8s_scale(resource_type: str, name: str, replicas: int, namespace: str = "default", session_id: str = None):
    """
    Scale a deployment, replicaset, statefulset, or replication controller.

    Args:
        resource_type: The type of resource to scale (deployment, replicaset, statefulset, replicationcontroller).
...
async def k8s_delete(
    ...
    Args:
        resource_type: The type of resource to delete (e.g., pods, deployments).
        name: The name of the resource to delete. If not specified, deletes resources matching the selectors.
        namespace: The namespace of the resource (default: default).
        label_selector: Label selector to filter resources (e.g., "app=nginx").
        field_selector: Field selector to filter resources (e.g., "metadata.name=nginx").
        all_namespaces: Whether to delete resources in all namespaces.
        cascade: Whether to cascade the deletion to dependent resources.
        grace_period: The grace period for the deletion in seconds.
        session_id: Kubernetes session ID for remote cluster (optional)
