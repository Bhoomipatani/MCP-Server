...
@mcp.tool()
def k8s_cp(src_path: str, dst_path: str, container: str = None, namespace: str = "default", session_id: str = None) -> dict:
    """
    Copy files and directories to and from containers.

    Args:
        src_path: The source path (local path or pod path in format pod_name:path)
        dst_path: The destination path (local path or pod path in format pod_name:path)
    ...
    Returns:
        Dictionary with copy operation result or error
    """
    try:
        ...
        logger.error(f"Error in k8s_cp: {exc}")
